...
class compute_cLCS_squeezelines():
  """
  ...
  def run(self):
    ...
    _,_,_,_,_,_,C11total,C22total,C12total,xspan,yspan,count = pickle.load(open(month_dirr+'TOT-'+m+'.p', 'rb'))
    N=count
    C11=C11total/N
    C22=C22total/N
    C12=C12total/N
    ArcLength=self.arclength
    self.pxt, self.pyt = self.squeezeline(C11,C12, C22, xspan[-1,:], yspan[:,-1], [0, ArcLength])
    pickle.dump([self.pxt,self.pyt], open(month_dirr+'/cLCS_'+m+'.p', 'wb'))
