...
def plot_colourline(x,y,c,cmap,ax=None,transform=None):
  #Plots LCSs using colouredlines to define intensity
    ...
    for i in np.arange(len(x)-1):
        ax.plot([x[i],x[i+1]], [y[i],y[i+1]], c=c[i], transform=transform, lw=.8)
    return
...
def cLCSrho_cartopy(dirr,monthvec,fig=None,ax=None, projection=None):
  ...
  m = '%02d' %  monthvec
  month_dirr=dirr+m+'/'
  lon,lat,_,sqrtlda2total,_,_,_,_,_,xspan,yspan,count = pickle.load(open(month_dirr+'TOT'+m+'.p', 'rb'))
  N=count
  xi=xspan[-1,:] #because the landmask was made NaN and the coast is on the left side
  yi=yspan[:,-1]
  X0, Y0 = np.meshgrid(xi,yi)
  X0 = X0.ravel()
  Y0 = Y0.ravel()
  pxt,pyt = pickle.load(open(month_dirr+'/cLCS_'+m+'.p', 'rb'))
  lonmin=lon.min()
  latmin=lat.min()
  lonmax=lon.max()
  latmax=lat.max()
